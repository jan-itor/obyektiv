...
class K2ModelItem extends K2Model
{
    ...
    function save($front = false)
    {
        ...
        if ( empty($row->modified) && $row->published > 0 )
        {
             
            function getCategory($item_id)
        {
            
            $query = "SELECT * FROM #__k2_categories WHERE id = ".(int)$item_id; 
            ...
            if ($result)
            {   
                if (intval($result[0]->parent) > 0) {
                    return getCategory($result[0]->parent);
                } else {
                    return $result;
                }
            }   
        }
             $vkPostCategory = getCategory($row->catid);
             $vkPostTags =  $this->getCurrentTags($row->id);
             $vkHashTags = ""; 
             foreach ($vkPostTags as &$tag ){
                 $vkHashTags .= '#'.$tag->name." "; 
             }                                 
             $PostTitle = $row->title;
             $PostLink = "http://obyektiv.press/".$vkPostCategory[0]->alias."/".$row->alias;
             $vkPostHashTags = '#'.$vkPostCategory[0]->name." ".$vkHashTags;
             $PostTitle .= "\n".$vkPostHashTags."\n".$PostLink;          
             $vkPostPicture = array("file1" =>  "@".$_SERVER['DOCUMENT_ROOT']."/media/k2/items/cache/".md5('Image'.$row->id)."_M.jpg"); 
             $fbPostDate = str_replace(" ", "T", $PublishUpDate).date("O", $PublishUpDate);          
            
            require $_SERVER['DOCUMENT_ROOT']."/vk_poster/export.php";
            require $_SERVER['DOCUMENT_ROOT']."/fb_poster/export.php";  
           
            $exportVk = new vkExport();
            $exportFb = new fbExport();
            
            $uplUrl = $exportVk->getWallUploadUrl(ConfigVk::$vkGroupID, ConfigVk::$vkAccessToken);

            $attachmentPhoto = $exportVk->GetPhotoInfo($vkPostPicture, $uplUrl, ConfigVk::$vkGroupID, ConfigVk::$vkAccessToken);
            
            if ( $row->created == $row->publish_up ){
            $resultVk = $exportVk->wallPost(ConfigVk::$_vkGroupID, ConfigVk::$vkAccessToken, $PostTitle, $PostLink, $attachmentPhoto);
            $resultFb = $exportFb->wallPost(ConfigFb::$fb_app_id, ConfigFb::$fb_app_secret, ConfigFb::$fb_page_id, ConfigFb::$fbToken, $PostLink, $PostTitle); 
            }else{
              $resultVk = $exportVk->wallPostDelay(ConfigVk::$_vkGroupID, ConfigVk::$vkAccessToken, strtotime($PublishUpDate), $PostTitle, $PostLink, $attachmentPhoto); 
              $resultFb = $exportFb->wallPostDelay(ConfigFb::$fb_app_id, ConfigFb::$fb_app_secret, ConfigFb::$fb_page_id, ConfigFb::$fbToken, $fbPostDate, $PostLink, $PostTitle);  
            }               
                    /*echo "<pre>"; print_r($resultFb); echo "</pre>";
                    die();*/               
        }
