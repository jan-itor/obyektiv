...
class SGAntiVirus {
	...
	public static function CheckStandaloneInstallation($access_key)
	{
        $access_key = trim($access_key);
        $webanalyze_path = JPATH_ROOT.'/webanalyze/';
        $antivirus_module = $webanalyze_path.'antivirus.php';
        $antivirus_config = $webanalyze_path.'antivirus_config.php';
        
        // Create folder /webanalyze/
        if (!file_exists($webanalyze_path)) 
        {
            if ( !mkdir($webanalyze_path) ) return "Can't create folder ".$webanalyze_path;
            self::CreateFile($webanalyze_path.'index.html', '');
        }
        
        // Check/Create file antivirus.php
        if (!file_exists($antivirus_module)) 
        {
            if ( !copy(dirname(__FILE__).'/antivirus.php', $antivirus_module) )
            {
                chmod($webanalyze_path, 755);
                
                if ( !copy(dirname(__FILE__).'/antivirus.php', $antivirus_module) ) return "Can't copy module antivirus.php";
            }
        }
        
        // Check/Create file antivirus_config.php
        if (!file_exists($antivirus_config)) 
        {
            // Create config file
            $file_content = '<?php'."\n".
            ...
            if (!self::CreateFile($antivirus_config, $file_content)) return "Can't save antivirus_config.php";    
        }
	...
	public static function GetLicenseInfo($domain, $access_key)
	{
		...
		$link = self::$siteguarding_server.'?action=licenseinfo&type=json';
        ...
        $msg = file_get_contents($link, false, $context);
	...
	public static function GetProgressInfo($domain, $access_key)
	{
        ...
        $link = self::$siteguarding_server.'?action=progressinfo&type=json';
	...
	public static function sendRegistration($domain, $email, $access_key = '', $errors = '')
	{
	    ...
	    $link = self::$siteguarding_server.'?action=register&type=json';
	    ...
	    //$link .= base64_encode(json_encode($data));
	    $msg = trim(file_get_contents($link, false, $context));
	    */
